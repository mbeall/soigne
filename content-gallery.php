...
<article id="post-<?php the_ID(); ?>" <?php post_class(); ?>>
	<header class="entry-header">
		<hgroup>
			...
			<h3 class="entry-format"><?php _e( 'Gallery', 'soigne' ); ?></h3>
		</hgroup>
	...
	<?php if ( is_search() ) : // Only display Excerpts for search pages ?>
		...
		<div class="entry-content">
			...
			<?php else : ?>
				...
				<p><em><?php printf( _n( 'This gallery contains <a %1$s>%2$s photo</a>.', 'This gallery contains <a %1$s>%2$s photos</a>.', $total_images, 'soigne' ),
						'href="' . esc_url( get_permalink() ) . '" title="' . sprintf( esc_attr__( 'Permalink to %s', 'soigne' ), the_title_attribute( 'echo=0' ) ) . '" rel="bookmark"',
